...
const ManagerProfile = () => {
  ...
  return (
    <div>
      <Breadcrumb items={[{ label: 'Manager', path: '/manager/dashboard' }, { label: 'Profile' }]} />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div className="card">
            ...
            <form onSubmit={handlePasswordChange}>
              <Input label="Current Password" name="currentPassword" type={showPwd ? 'text' : 'password'} value={pwdForm.currentPassword} onChange={e => setPwdForm(p => ({ ...p, currentPassword: e.target.value }))} placeholder="Current password" iconRight={showPwd ? '🙈' : '👁️'} onIconRightClick={() => setShowPwd(p => !p)} />
              <Input label="New Password" name="newPassword" type={showPwd ? 'text' : 'password'} value={pwdForm.newPassword} onChange={e => setPwdForm(p => ({ ...p, newPassword: e.target.value }))} placeholder="Min 8 characters" />
              <Input label="Confirm Password" name="confirmPassword" type={showPwd ? 'text' : 'password'} value={pwdForm.confirmPassword} onChange={e => setPwdForm(p => ({ ...p, confirmPassword: e.target.value }))} placeholder="Repeat new password" />
