...
const AdminInventory = () => {
  ...
  const fetchInventory = useCallback(async () => {
    ...
    try {
      ...
      setInventory(data)
      const cats = [...new Set(data.map(i => i.categoryName).filter(Boolean))]
      setCategories(cats)
    } catch {
  ...
  const totalAvailable = inventory.reduce((s, i) => s + (i.availableQuantity ?? i.available ?? 0), 0)
  const totalReserved = inventory.reduce((s, i) => s + (i.reservedQuantity ?? i.reserved ?? 0), 0)
  const totalDamaged = inventory.reduce((s, i) => s + (i.damagedQuantity ?? i.damaged ?? 0), 0)
  const lowStockCount = inventory.filter(i => i.status === 'low').length

  return (
    <div>
      <Breadcrumb items={[{ label: 'Admin', path: '/admin/dashboard' }, { label: 'Inventory' }]} />
      <div className="page-header">
        <div><h1 className="page-title">Inventory Management</h1><p className="page-subtitle">Real-time stock overview</p></div>
      </div>

      <div className="stat-cards" style={{ marginBottom: 24 }}>
        {[
          { icon: '✅', label: 'Available Stock', value: formatNumber(totalAvailable), variant: 'success' },
          { icon: '🔒', label: 'Reserved Stock', value: formatNumber(totalReserved), variant: 'info' },
          { icon: '⚠️', label: 'Damaged Stock', value: formatNumber(totalDamaged), variant: 'warning' },
          { icon: '🔴', label: 'Low Stock Items', value: lowStockCount, variant: 'danger' },
        ].map((s, i) => (
      ...
      <div className="table-container">
        ...
        {loading ? <table><tbody><SkeletonTable rows={6} cols={8} /></tbody></table> : filtered.length === 0 ? (
          ...
          <>
            <table>
              <thead><tr><th>#</th><th>Product</th><th>SKU</th><th>Category</th><th>Available</th><th>Reserved</th><th>Damaged</th><th>Status</th><th>Last Updated</th><th>Action</th></tr></thead>
              <tbody>
                {paginated.map((item, i) => {
                  ...
                  return (
                    <tr key={item.id}>
                      ...
                      <td className="text-muted">{formatDate(item.lastUpdated ?? item.updatedAt)}</td>
                      <td>
                        <button className="btn btn-ghost btn-sm" onClick={() => setViewItem(item)}>👁️ Details</button>
                      </td>
                    </tr>
                  )
                })}
